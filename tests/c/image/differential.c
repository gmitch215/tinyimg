...
/**
 * @brief A contrast change about mid gray is an affine function of the channel.
 *
...
/**
 * @brief Our JPEG decode against a third party's, carried in an uncompressed
 * TIFF.
 *
 * The strongest reference in the set, because nothing here produced it.
 * `dartmouth.tiff` is `dartmouth.jpg` decoded by an unrelated tool and written
 * out losslessly, so an exact match says our IDCT, our chroma upsampling and
 * our color conversion all agree with a standard decoder sample for sample.
 * Every other codec reference is generated by scripts/fixtures.ts, which means
 * it shares whatever magick and cjpeg assume.
 *
 * Asserted exact rather than against a floor because it measured exact: worst
 * channel delta 0 over all 140,250 samples. A floor here would let a real
 * regression through, and rounding is the only thing that could have differed.
 *
 * It checks the TIFF path at the same time, since a wrong strip offset or row
 * stride on that side would break the match just as surely.
 */
static int cross_container(void) {
    int failures = 0;
    TinyImage jpeg;
    TinyImage tiff;

    printf("  jpeg decode against an independent tiff of the same picture\n");

    if (!load(&jpeg, "dartmouth.jpg")) return 1;
    if (!load(&tiff, "dartmouth.tiff")) {
        tiny_image_destroy(&jpeg);
        return 1;
    }

    failures += assertImageEquals(&jpeg, &tiff);

    tiny_image_destroy(&jpeg);
    tiny_image_destroy(&tiff);
    return failures;
}
