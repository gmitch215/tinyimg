...
/**
 * @brief Builds the tables the library derives at runtime and installs the
 * default heap.
 *
 * Idempotent, and every entry point that needs it calls it, so nothing breaks
 * if a host never calls it. A host that would rather pay the cost at startup
 * than on the first request calls it once after instantiating the module.
 */
void tiny_init(void);
