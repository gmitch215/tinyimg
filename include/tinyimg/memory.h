 ...
 *
 * A default of each kind is compiled in, which is what makes text, profile
 * conversion and face detection work off a bare instantiate. A host that wants
 * a different face, a wider glyph set or its own cascade loads one at runtime
 * from a wrangler Data module or a bucket; the library only ever sees bytes
 * already in linear memory.
 */
...
/**
 * @brief Looks up a blob.
 *
 * Resident blobs first, then the builtins, so a host that loads its own face
 * under an id shadows nothing else and a host that loads none still resolves
 * `sans`.
 *
 ...
 *
 * **A resident blob of a kind hides every builtin of that kind.** Loading one
 * cascade means a search runs that cascade and not the two that ship, which is
 * what a host replacing a default expects; loading none means it runs both
 * builtins. tiny_blob_get resolves by name and is not shadowed this way.
 *
 ...
 *
 * A builtin is in the module's data section, so it cannot be released and
 * reports TINYIMG_ERR_NOT_FOUND.
 *
...
/**
 * @brief A blob compiled into the module.
 *
 * The defaults tinyimg ships: one Latin face, four ICC profiles and two LBP
 * cascades, about 45 KB of the module. They need no host wiring, so text
 * drawing, profile conversion and face detection all work off a bare
 * instantiate.
 *
 * @note The face and the cascades are third-party data. Their notices are in
 * `LICENSES/`.
 */
typedef struct {
    /** What the bytes are. */
    TinyBlobKind kind;
    /** Name it resolves under, the same id the published blob carries. */
    const char* id;
    /** The bytes, in the module's data section. */
    const uint8_t* data;
    /** Number of bytes. */
    size_t size;
} TinyBlobBuiltin;
...
/**
 * @brief The builtin table, generated by `scripts/blobs-builtin.ts`.
 */
extern const TinyBlobBuiltin tiny_blob_builtins[];

/**
 * @brief How many entries tiny_blob_builtins holds.
 */
extern const uint32_t tiny_blob_builtin_count;

/**
 * @brief Walks the builtins of one kind.
 *
 * Reports what the module carries whether or not anything is resident, which
 * is what a host listing the available faces wants. Resolution is the other
 * question and tiny_blob_get answers it.
 *
 * @param kind What to walk.
 * @param index Which one, from zero.
 * @param id Receives the id. May be NULL.
 * @param size Receives the byte length. May be NULL.
 * @return const uint8_t* The bytes, or NULL once `index` is past the last one.
 */
